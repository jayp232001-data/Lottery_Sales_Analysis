...
# # Step 1: Read input data from AWS Glue Data Catalog table "lottery_sales" in "test_db"
...
# Step 2: Standardize column names by stripping whitespace, replacing spaces with underscores, and converting to lowercase
...
# Step 4: Cast columns to appropriate data types for accurate processing and calculations
...
# Step 11: Add retailer_group column — "Self-Owned Retailer" if retailer owns itself, else use owning entity name
df = df.withColumn(
    "retailer_group",
    when(
...
# Step 12: Add region column by mapping retailer_location_county to predefined Texas regions (panhandle, north texas, etc.)
...
# Step 14: Drop unwanted or redundant columns to clean dataset
columns_to_drop = [
...
# Step 9: Convert cancelled, promotional, and ticket returns amounts to positive values using absolute function
...
# Step 10: Derive number of tickets returned and sold by dividing amounts by ticket_price (handle divide by zero by conditional)
df = df.withColumn(
...
#Yash
# Step 7: Add flag column 'is_negative_sale' to identify rows where net ticket sales amount is negative (e.g., refunds)
df = df.withColumn("is_negative_sale", when(col("net_ticket_sales_amount") < 0, 1).otherwise(0))


# Step 8: Add string version of ticket_price for any string operations or output formatting
...
output_path = "s3://lotterypopulationfinal/outputone/"
